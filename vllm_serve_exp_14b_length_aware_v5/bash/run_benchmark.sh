#!/usr/bin/env bash
# ===== User Config =====
RUN_TAG="${RUN_TAG:-14b_length_aware_v5}"
HOST="${HOST:-127.0.0.1}"
PORT="${PORT:-8070}"
BASE_URL="${BASE_URL:-http://$HOST:$PORT}"
SCHEDULER_STATS_URL="${SCHEDULER_STATS_URL:-$BASE_URL/scheduler_stats}"
POLICY_TAG="${POLICY_TAG:-length_aware_v5}"
...
PROMPT_FILE="${PROMPT_FILE:-vllm_serve_exp_14b_length_aware_v5/data/mixed_prompts_30s70l.jsonl}"
...
RESULT_NAMESPACE="${RESULT_NAMESPACE:-vllm_serve_14b_length_aware_v5}"
...
python -m vllm_serve_exp_14b_length_aware_v5.client_benchmark \
  --base_url "$BASE_URL" \
