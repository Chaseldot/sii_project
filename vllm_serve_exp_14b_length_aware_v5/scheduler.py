...
class SchedulerConfig:
    ...
    long_head_wait_guard_ms: float = 45000.0
    long_queue_len_guard: int = 96
...
class LengthAwareScheduler:
    ...
    _long_guard_dispatches: int = field(default=0, init=False)
    ...
    def snapshot(self) -> dict:
        with self._condition:
            ...
            return {
                ...
                "long_head_wait_guard_ms": self.config.long_head_wait_guard_ms,
                "long_queue_len_guard": self.config.long_queue_len_guard,
                ...
                "scheduler_long_guard_dispatches": self._long_guard_dispatches,
    ...
    def _select_next_ticket_locked(self) -> QueueTicket | None:
        if self.config.policy == "fifo":
            ...
            self._after_dispatch_locked(ticket.bucket)
            return ticket
    ...
    def _select_length_aware_ticket_locked(self) -> QueueTicket | None:
        ...
        if has_long and not has_short:
            ticket = self._dispatch_long_locked(reason="only_long")
            return ticket
        if has_short and not has_long:
            ticket = self._dispatch_short_locked(reason="only_short")
            return ticket

        if self._consecutive_short >= self.config.max_consecutive_short:
            ticket = self._dispatch_long_locked(reason="max_consecutive_short")
            return ticket

        long_guard_reason = self._get_long_guard_reason_locked()
        if long_guard_reason is not None:
            self._long_guard_dispatches += 1
            ticket = self._dispatch_long_locked(reason=long_guard_reason)
            return ticket

        arrival_metrics = self._compute_arrival_metrics_locked()
        target_short_share = self._compute_target_short_share_locked(arrival_metrics)
        ...
        if self._short_credit >= self._long_credit:
            ticket = self._dispatch_short_locked(reason="fair_share_short")
            self._short_credit -= 1.0
        else:
            ticket = self._dispatch_long_locked(reason="fair_share_long")
    ...
    def _dispatch_short_locked(self, reason: str) -> QueueTicket:
        ...
        self._after_dispatch_locked("short")
        return ticket
    ...
    def _dispatch_long_locked(self, reason: str) -> QueueTicket:
        ...
        self._last_dispatch_reason = reason
        self._after_dispatch_locked("long")
        return ticket

    def _after_dispatch_locked(self, bucket: str) -> None:
    ...
    def _compute_target_short_share_locked(self, arrival_metrics: dict) -> float:
        target = arrival_metrics["short_ratio"] + self.config.target_short_share_bonus
    ...
    def _get_long_guard_reason_locked(self) -> str | None:
        long_head_wait_ms = self._head_wait_ms_locked(self._long_queue)
        if long_head_wait_ms >= self.config.long_head_wait_guard_ms:
            return "long_head_wait_guard"
        if (
            len(self._long_queue) >= self.config.long_queue_len_guard
            and len(self._long_queue) >= len(self._short_queue)
        ):
            return "long_queue_len_guard"
        return None
        ...
        self._policy_updates.append(
            {
                ...
                "long_guard_dispatches": self._long_guard_dispatches,
            }
        )
