...
from .engine import EngineConfig, VLLM14BEngine
...
def parse_args():
    parser = argparse.ArgumentParser(description="vLLM 14B 单条离线推理验证")
    parser.add_argument("--model_path", type=str, required=True, help="模型本地路径")
    parser.add_argument(
        "--prompt",
        type=str,
        default="请系统解释大模型推理中的 PagedAttention、KV Cache 与动态 Batch 的关系。",
        help="测试 prompt",
    )
