    ...
    class LengthAwareProxyHandler(BaseHTTPRequestHandler):
        ...
        def do_GET(self) -> None:
            if self.path == "/healthz":
                self._send_json({"ok": True, "service": "vllm_122b_length_aware_proxy_v6"})
                return
...
def parse_args():
    parser = argparse.ArgumentParser(description="Ratio-aware length-aware proxy for 122B vLLM serve")
    parser.add_argument("--host", type=str, default="127.0.0.1")
    parser.add_argument("--port", type=int, default=8185)
    parser.add_argument("--backend_base_url", type=str, required=True)
    parser.add_argument("--policy", type=str, default="length_aware_v6", choices=["fifo", "length_aware_v6"])
    ...
    parser.add_argument("--queue_ratio_control_gain", type=float, default=1.0)
    parser.add_argument("--queue_ratio_margin", type=float, default=0.08)
    parser.add_argument("--max_ratio_adjustment", type=float, default=0.2)
