    ...
    class LengthAwareProxyHandler(BaseHTTPRequestHandler):
        ...
        def do_GET(self) -> None:
            if self.path == "/healthz":
                self._send_json({"ok": True, "service": "vllm_length_aware_proxy_v6_1"})
                return
...
def parse_args():
    parser = argparse.ArgumentParser(description="Ratio-aware length-aware proxy for vLLM serve")
    parser.add_argument("--host", type=str, default="127.0.0.1")
    parser.add_argument("--port", type=int, default=8086)
    parser.add_argument("--backend_base_url", type=str, required=True)
    parser.add_argument("--policy", type=str, default="length_aware_v6_1", choices=["fifo", "length_aware_v6_1"])
    ...
    parser.add_argument("--target_short_share_bonus", type=float, default=0.18)
    parser.add_argument("--min_short_share", type=float, default=0.42)
    parser.add_argument("--max_short_share", type=float, default=0.72)
    parser.add_argument("--queue_ratio_control_gain", type=float, default=0.8)
    parser.add_argument("--queue_ratio_margin", type=float, default=0.12)
    parser.add_argument("--max_ratio_adjustment", type=float, default=0.12)
...
def main():
    ...
    scheduler = LengthAwareScheduler(
        SchedulerConfig(
            ...
            queue_ratio_control_gain=args.queue_ratio_control_gain,
            queue_ratio_margin=args.queue_ratio_margin,
            max_ratio_adjustment=args.max_ratio_adjustment,
        )
    )
    ...
    print(f"[INFO] queue_ratio_control_gain={args.queue_ratio_control_gain}")
    print(f"[INFO] queue_ratio_margin={args.queue_ratio_margin}")
    print(f"[INFO] max_ratio_adjustment={args.max_ratio_adjustment}")
