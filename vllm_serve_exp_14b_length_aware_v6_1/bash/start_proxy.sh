#!/usr/bin/env bash
# ===== User Config =====
# 这是 ratio-aware constrained fair scheduler 参数微调版。
# - POLICY: fifo / length_aware_v6_1
...
POLICY="${POLICY:-length_aware_v6_1}"
...
TARGET_SHORT_SHARE_BONUS="${TARGET_SHORT_SHARE_BONUS:-0.18}"
MIN_SHORT_SHARE="${MIN_SHORT_SHARE:-0.42}"
MAX_SHORT_SHARE="${MAX_SHORT_SHARE:-0.72}"
QUEUE_RATIO_CONTROL_GAIN="${QUEUE_RATIO_CONTROL_GAIN:-0.8}"
QUEUE_RATIO_MARGIN="${QUEUE_RATIO_MARGIN:-0.12}"
MAX_RATIO_ADJUSTMENT="${MAX_RATIO_ADJUSTMENT:-0.12}"
# ===== End User Config =====
set -euo pipefail
...
python -m vllm_serve_exp_14b_length_aware_v6_1.proxy \
  --host "$HOST" \
  --port "$PORT" \
