...
class SchedulerConfig:
    policy: str = "length_aware_v7"
    short_threshold_chars: int = 256
    max_consecutive_short: int = 4
    max_consecutive_long: int = 4
    max_active_requests: int = 64
    max_queue_wait_sec: float = 300.0
    arrival_window_size: int = 256
    control_update_interval: int = 64
    target_short_share_bonus: float = 0.1
    min_short_share: float = 0.35
    max_short_share: float = 0.65
    pressure_eps: float = 0.05
    pressure_gain: float = 0.6
    pressure_margin: float = 0.15
    max_pressure_adjustment: float = 0.15
...
class LengthAwareScheduler:
    ...
    _consecutive_long: int = field(default=0, init=False)
    ...
    _pressure_gap_current: float = field(default=0.0, init=False)
    _pressure_gap_sum: float = field(default=0.0, init=False)
    _pressure_gap_samples: int = field(default=0, init=False)
    _pressure_adjustment_current: float = field(default=0.0, init=False)
    _pressure_adjustment_sum: float = field(default=0.0, init=False)
    _pressure_adjustment_samples: int = field(default=0, init=False)
    _pressure_bias_to_short_dispatches: int = field(default=0, init=False)
    _pressure_bias_to_long_dispatches: int = field(default=0, init=False)
    ...
    _last_pressure_metrics: dict = field(default_factory=dict, init=False)
    ...
    def snapshot(self) -> dict:
        with self._condition:
            ...
            pressure_metrics = self._compute_pressure_metrics_locked(arrival_metrics, queue_metrics)
            ...
            pressure_gap_avg = 0.0
            if self._pressure_gap_samples > 0:
                pressure_gap_avg = self._pressure_gap_sum / self._pressure_gap_samples

            pressure_adjustment_avg = 0.0
            if self._pressure_adjustment_samples > 0:
                pressure_adjustment_avg = self._pressure_adjustment_sum / self._pressure_adjustment_samples
            ...
            return {
                ...
                "max_consecutive_long": self.config.max_consecutive_long,
                ...
                "pressure_eps": self.config.pressure_eps,
                "pressure_gain": self.config.pressure_gain,
                "pressure_margin": self.config.pressure_margin,
                "max_pressure_adjustment": self.config.max_pressure_adjustment,
                ...
                "scheduler_short_pressure_current": round(pressure_metrics["short_pressure"], 4),
                "scheduler_long_pressure_current": round(pressure_metrics["long_pressure"], 4),
                "scheduler_pressure_gap_current": round(self._pressure_gap_current, 4),
                "scheduler_pressure_gap_avg": round(pressure_gap_avg, 4),
                ...
                "scheduler_pressure_adjustment_current": round(self._pressure_adjustment_current, 4),
                "scheduler_pressure_adjustment_avg": round(pressure_adjustment_avg, 4),
                "scheduler_short_credit": round(self._short_credit, 4),
                "scheduler_long_credit": round(self._long_credit, 4),
                "scheduler_pressure_bias_to_short_dispatches": self._pressure_bias_to_short_dispatches,
                "scheduler_pressure_bias_to_long_dispatches": self._pressure_bias_to_long_dispatches,
                ...
                "scheduler_last_pressure_metrics": self._last_pressure_metrics,
    ...
    def _select_next_ticket_locked(self) -> QueueTicket | None:
        if self.config.policy == "fifo":
            ...
            return ticket
        return self._select_pressure_aware_ticket_locked()

    def _select_pressure_aware_ticket_locked(self) -> QueueTicket | None:
        ...
        if self._consecutive_long >= self.config.max_consecutive_long:
            return self._dispatch_short_locked(reason="max_consecutive_long")

        arrival_metrics = self._compute_arrival_metrics_locked()
        queue_metrics = self._compute_queue_metrics_locked()
        pressure_metrics = self._compute_pressure_metrics_locked(arrival_metrics, queue_metrics)

        base_target = self._clamp_float(
            arrival_metrics["short_ratio"] + self.config.target_short_share_bonus,
            self.config.min_short_share,
            self.config.max_short_share,
        )
        adjustment = 0.0
        reason = "pressure_balanced"
        if pressure_metrics["gap"] > self.config.pressure_margin:
            adjustment = min(
                self.config.max_pressure_adjustment,
                pressure_metrics["gap"] * self.config.pressure_gain,
            )
            reason = "pressure_bias_short"
        elif pressure_metrics["gap"] < -self.config.pressure_margin:
            adjustment = max(
                -self.config.max_pressure_adjustment,
                pressure_metrics["gap"] * self.config.pressure_gain,
            )
            reason = "pressure_bias_long"

        target_short_share = self._clamp_float(
        ...
        self._pressure_gap_current = pressure_metrics["gap"]
        self._pressure_gap_sum += pressure_metrics["gap"]
        self._pressure_gap_samples += 1
        self._pressure_adjustment_current = adjustment
        self._pressure_adjustment_sum += adjustment
        self._pressure_adjustment_samples += 1
        self._last_arrival_metrics = {
            **arrival_metrics,
            "base_target_short_share": round(base_target, 4),
            "target_short_share": round(target_short_share, 4),
        }
        self._last_queue_metrics = queue_metrics
        self._last_pressure_metrics = {
            **pressure_metrics,
        ...
        if self._short_credit >= self._long_credit:
            if reason == "pressure_bias_short":
                self._pressure_bias_to_short_dispatches += 1
            return self._dispatch_short_locked(reason=reason)

        if reason == "pressure_bias_long":
            self._pressure_bias_to_long_dispatches += 1
        return self._dispatch_long_locked(reason=reason)
    ...
    def _compute_arrival_metrics_locked(self) -> dict:
        ...
        return {
            ...
            "long_ratio": 1.0 - short_ratio if total > 0 else 0.0,
        }
    ...
    def _compute_pressure_metrics_locked(self, arrival_metrics: dict, queue_metrics: dict) -> dict:
        short_pressure = queue_metrics["short_ratio"] / max(arrival_metrics["short_ratio"], self.config.pressure_eps)
        long_pressure = queue_metrics["long_ratio"] / max(arrival_metrics["long_ratio"], self.config.pressure_eps)
        gap = short_pressure - long_pressure
        return {
    ...
    def _record_control_update_locked(self) -> None:
        ...
        reason = "pressure_balanced"
        if pressure_metrics["gap"] > self.config.pressure_margin:
            adjustment = min(self.config.max_pressure_adjustment, pressure_metrics["gap"] * self.config.pressure_gain)
            reason = "pressure_bias_short"
        elif pressure_metrics["gap"] < -self.config.pressure_margin:
            adjustment = max(-self.config.max_pressure_adjustment, pressure_metrics["gap"] * self.config.pressure_gain)
            reason = "pressure_bias_long"
        target_short_share = self._clamp_float(
            base_target + adjustment,
            self.config.min_short_share,
        ...
        self._policy_updates.append(
            {
                ...
                "short_pressure": round(pressure_metrics["short_pressure"], 4),
                "long_pressure": round(pressure_metrics["long_pressure"], 4),
                "pressure_gap": round(pressure_metrics["gap"], 4),
                "base_target_short_share": round(base_target, 4),
                "target_short_share": round(target_short_share, 4),
                "actual_short_share": round(actual_short_share, 4),
                "pressure_adjustment": round(adjustment, 4),
