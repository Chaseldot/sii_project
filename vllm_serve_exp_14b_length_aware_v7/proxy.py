    ...
    class LengthAwareProxyHandler(BaseHTTPRequestHandler):
        ...
        def do_GET(self) -> None:
            if self.path == "/healthz":
                self._send_json({"ok": True, "service": "vllm_length_aware_proxy_v7"})
                return
...
def parse_args():
    parser = argparse.ArgumentParser(description="Pressure-aware length-aware proxy for vLLM serve")
    parser.add_argument("--host", type=str, default="127.0.0.1")
    parser.add_argument("--port", type=int, default=8087)
    parser.add_argument("--backend_base_url", type=str, required=True)
    parser.add_argument("--policy", type=str, default="length_aware_v7", choices=["fifo", "length_aware_v7"])
    ...
    parser.add_argument("--max_consecutive_long", type=int, default=4)
    ...
    parser.add_argument("--target_short_share_bonus", type=float, default=0.1)
    parser.add_argument("--min_short_share", type=float, default=0.35)
    parser.add_argument("--max_short_share", type=float, default=0.65)
    parser.add_argument("--pressure_eps", type=float, default=0.05)
    parser.add_argument("--pressure_gain", type=float, default=0.6)
    parser.add_argument("--pressure_margin", type=float, default=0.15)
    parser.add_argument("--max_pressure_adjustment", type=float, default=0.15)
...
def main():
    ...
    scheduler = LengthAwareScheduler(
        SchedulerConfig(
            ...
            max_consecutive_long=args.max_consecutive_long,
            ...
            pressure_eps=args.pressure_eps,
            pressure_gain=args.pressure_gain,
            pressure_margin=args.pressure_margin,
            max_pressure_adjustment=args.max_pressure_adjustment,
        )
    )
    ...
    print(f"[INFO] max_consecutive_long={args.max_consecutive_long}")
    ...
    print(f"[INFO] pressure_eps={args.pressure_eps}")
    print(f"[INFO] pressure_gain={args.pressure_gain}")
    print(f"[INFO] pressure_margin={args.pressure_margin}")
    print(f"[INFO] max_pressure_adjustment={args.max_pressure_adjustment}")
