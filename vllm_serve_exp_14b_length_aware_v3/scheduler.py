...
class LengthAwareScheduler:
    ...
    def _record_completion_locked(self, ticket: dict, ttft_ms: float, latency_ms: float) -> None:
        ...
        if self.config.policy != "length_aware_v3":
            return
        ...
        self._apply_adaptation_locked(metrics)
    ...
    def _compute_window_metrics_locked(self) -> dict:
        ...
        return {
            ...
            "short_avg_prompt_chars": round(avg(short_rows, "prompt_chars"), 2),
            "long_avg_prompt_chars": round(avg(long_rows, "prompt_chars"), 2),
    ...
    def _apply_adaptation_locked(self, metrics: dict) -> None:
        short_ratio = metrics["short_ratio"]
        desired_weight = 1
        desired_consecutive = 3

        if short_ratio <= 0.30:
            desired_weight = 3
            desired_consecutive = 6
        elif short_ratio <= 0.50:
            desired_weight = 2
            desired_consecutive = 4

        short_p95_ttft = metrics["short_p95_ttft_ms"]
        long_p95_ttft = metrics["long_p95_ttft_ms"]
        short_p95_latency = metrics["short_p95_latency_ms"]
        long_p95_latency = metrics["long_p95_latency_ms"]

        if short_p95_ttft > 0 and long_p95_ttft > 0 and short_p95_ttft >= long_p95_ttft * 0.90:
            desired_weight += 1
            desired_consecutive += 1

        if short_p95_latency > 0 and long_p95_latency > short_p95_latency * 2.20:
            desired_weight -= 1
            desired_consecutive -= 1

        desired_weight = self._clamp(
            desired_weight,
            self.config.min_short_weight,
            self.config.max_short_weight,
        )
        desired_consecutive = self._clamp(
            desired_consecutive,
            self.config.min_max_consecutive_short,
            self.config.max_max_consecutive_short,
        )
        ...
        new_weight = self._step_towards(old_weight, desired_weight)
        new_consecutive = self._step_towards(old_consecutive, desired_consecutive)
        ...
        self._policy_updates.append(
            {
                ...
                "window_long_p95_ttft_ms": metrics["long_p95_ttft_ms"],
                "window_short_p95_latency_ms": metrics["short_p95_latency_ms"],
