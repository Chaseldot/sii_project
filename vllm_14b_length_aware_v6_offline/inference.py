...
def render_result(result: dict, peak_gpu_mem_gb: float) -> str:
    return "\n".join(
        [
            "",
            "=" * 64,
            " vLLM 14B Length-Aware V6 Offline 推理结果",
            "=" * 64,
            f"  输入   : {result['prompt']}",
            f"  输出   : {result['output']}",
            "-" * 64,
...
def parse_args():
    parser = argparse.ArgumentParser(description="vLLM 14B length-aware v6 单条离线推理验证")
    parser.add_argument("--model_path", type=str, required=True, help="模型本地路径")
    parser.add_argument(
        "--prompt",
        type=str,
        default="请系统解释离线批量推理中，长度感知 batch 组装与 KV Cache 利用之间的关系。",
        help="测试 prompt",
    )
